      ...
      <main className="pt-20 sm:pt-24 pb-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="mb-4 sm:mb-6 max-w-4xl mx-auto">
          <div className="relative">
            <input
              ...
              className="w-full px-4 sm:px-5 py-2.5 sm:py-3 pl-10 sm:pl-12 pr-4 rounded-full bg-white/80 backdrop-blur-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent shadow-sm text-sm sm:text-base"
            />
            <svg
              ...
              {filteredProducts.length > 0 ? (
                <div>
                  <p className="text-sm text-gray-600 mb-3">
                    พบสินค้าที่ชื่อคล้ายกัน <span className="font-bold text-purple-600">{filteredProducts.length}</span> รายการ
                  </p>
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {filteredProducts.map((product, index) => (
                      <div
                        key={product.id}
                        className="flex items-center gap-3 p-2 hover:bg-purple-50 rounded-lg transition-colors cursor-pointer"
                        onClick={() => router.push(`/product?pname=${product.name}`)}
                      >
                        <div className="flex-shrink-0 w-8 h-8 bg-gradient-to-br from-purple-500 to-purple-300 rounded-full flex items-center justify-center text-white text-xs font-semibold">
                          {index + 1}
                        </div>
        ...
        <div className="max-w-4xl mx-auto">
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">
            {search ? `ผลการค้นหา (${filteredProducts.length})` : "สินค้าแนะนำ"}
          </h2>
          ...
          {filteredProducts.length === 0 ? (
            <div className="text-center py-12 bg-white/50 rounded-3xl">
              ...
              <button
                ...
                className="px-6 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-full text-sm transition-colors"
              >
                ล้างการค้นหา
              </button>
            </div>
